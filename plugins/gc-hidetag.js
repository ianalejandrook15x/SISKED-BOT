...
const handler = async (m, {conn, text, participants, isOwner, isAdmin}) => {
  try {
    ...
    if ((isMedia && quoted.mtype === 'imageMessage') && htextos) {
      ...
      await conn.relayMessage(m.chat, {extendedTextMessage: {text: `${masss}\n${htextos}\n`, ...{contextInfo: {mentionedJid: users, externalAdReply: {thumbnail: imagen1, sourceUrl: 'https://chat.whatsapp.com/CN3YieShy2S3LnVRJ7aPPM'}}}}}, {});
    }
  }
};
handler.command = /^(hidetag|hidetah|hidet)$/i;
handler.group = true;
handler.admin = true;
